  ...
  for (const streetNames of streetNameTests) {
    for (const streetName of streetNames) {
      await it(`Gets alternative spellings for "${streetName}"`, () => {
        const alternativeSpellings = getAlternateStreetNameSpellings(
          streetName,
          'upper'
        )

        console.log(
          `Alternative spellings for "${streetName}":`,
          alternativeSpellings
        )

        for (const expectedAlternativeSpelling of streetNames) {
          assert.ok(
            alternativeSpellings.includes(
              expectedAlternativeSpelling.toUpperCase()
            ),
            `Expected "${expectedAlternativeSpelling}" to be included in alternative spellings for "${streetName}"`
          )
        }
      })
    }
